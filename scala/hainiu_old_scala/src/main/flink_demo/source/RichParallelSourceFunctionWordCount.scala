...
// 4.自定义 source (并行 富有的) 带有 getRuntimeContext 进行操作
//    继承RichParallelSourceFunction(富有的)
object RichParallelSourceFunctionWordCount {
  def main(args: Array[String]): Unit = {
    ...
    val windowCounts = text.setParallelism(1).flatMap(new FlatMapFunction[String, (String, Int)] {
...
//    breakable{
      while(isCancel){
        println("getTaskName：" + getRuntimeContext.getTaskName)
        println("getIndexOfThisSubtask：" + getRuntimeContext.getIndexOfThisSubtask)
