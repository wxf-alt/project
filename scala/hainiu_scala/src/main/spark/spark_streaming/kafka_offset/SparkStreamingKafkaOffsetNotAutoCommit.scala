...
object SparkStreamingKafkaOffsetNotAutoCommit {
  def main(args: Array[String]): Unit = {
    ...
    val conf: SparkConf = new SparkConf().setAppName("sparkstreamingkafkaoffset").setMaster("local[*]")
    ...
    val kafkaParams: Map[String, Object] = Map[String, Object](
      "bootstrap.servers" -> "s2.hadoop:9092,s3.hadoop:9092,s4.hadoop:9092",
    ...
    stream.foreachRDD { rdd =>
      ...
      for (o <- offsetRanges) {
        val zkPath: String = s"partitioner:${o.partition}_offset:${o.untilOffset.toString},"
        print(zkPath)
      }
