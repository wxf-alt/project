...
object SparkStreamingKafkaOffsetZKTransform {
  def main(args: Array[String]): Unit = {
    ...
    kafkaParams += "bootstrap.servers" -> brokerList
    kafkaParams += "group.id" -> group
...
//    kafkaParams += "enable.auto.commit" -> (false: java.lang.Boolean)
    ...
    //定义一个空的 kafkaStream流，之后根据是否有历史的偏移量进行选择
    var kafkaStream: InputDStream[ConsumerRecord[String, String]] = null

    //如果存在历史的偏移量，那使用fromOffsets来存放存储在zk中的每个TopicPartition对应的offset
    var fromOffsets: mutable.Map[TopicPartition, Long] = new mutable.HashMap[TopicPartition, Long]
    ...
    if (children > 0) {
      ...
      kafkaStream = KafkaUtils.createDirectStream[String, String](ssc, LocationStrategies.PreferConsistent, ConsumerStrategies.Subscribe[String, String](topics, kafkaParams, fromOffsets))
    } else {
