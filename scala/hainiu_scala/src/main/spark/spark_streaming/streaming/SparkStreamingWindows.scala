...
object SparkStreamingWindows {
  def main(args: Array[String]): Unit = {
    val conf: SparkConf = new SparkConf().setMaster("local[2]").setAppName("SparkStreamingWindows")
    val ssc: StreamingContext = new StreamingContext(conf, Seconds(2))
    val hadoopConfiguration: Configuration = ssc.sparkContext.hadoopConfiguration
    hadoopConfiguration.set("fs.defaultFS", "file:///")

    val checkPoint = "/Users/leohe/Data/output/sparkstreamingsocketportupdatestatewindows"
    ...
    val reduceByKey: DStream[(String, Int)] = mapToPair.reduceByKey(_ + _).window(Durations.seconds(6),Durations.seconds(4))
