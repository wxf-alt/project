...
class KafkaProducerWithDSer extends Actor {
...
class KafkaConsumerWithDSer extends Actor {
...
//    this.consumer.assign(java.util.Arrays.asList(new TopicPartition(topic, 1)))
        val topicPartition: TopicPartition = new TopicPartition(topic, 2)
...
object KafkaTestWithDSer {
  def main(args: Array[String]): Unit = {
    val topic: String = "hainiu_test2"
    val kafkaProducerWithDSer: KafkaProducerWithDSer = new KafkaProducerWithDSer(topic)
    val kafkaConsumerWithDSer: KafkaConsumerWithDSer = new KafkaConsumerWithDSer(topic)
    kafkaProducerWithDSer.start()
    kafkaConsumerWithDSer.start()
  }
}
