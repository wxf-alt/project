...
//    prop.put("zookeeper.session.timeout.ms", "600000")
  ...
  override def act(): Unit = {
    ...
    val stream: KafkaStream[Array[Byte], Array[Byte]] = createMessageStreams.get(topic).get(0)
...
object KafkaTest {
  def main(args: Array[String]): Unit = {
    ...
    val producer: HainiuProducer = new HainiuProducer(topic)
    val consumer: HainiuConsumer = new HainiuConsumer(topic)
    consumer.start()
    producer.start()
  }
}
